(() => {
  ...
  var VideoGalleryComponent = class extends HTMLElement {
    constructor() {
      ...
      this.productDataMap = /* @__PURE__ */ new Map();
    }
    async connectedCallback() {
      this.injectStyles();
      this.render();
    ...
    async fetchProductData() {
      const productIds = this.videos.map((v) => v.productId).filter((id) => id);
    ...
    renderProductInfoCards() {
      this.videos.forEach((video) => {
        ...
        const priceHtml = product.is_on_sale ? `<span class="product-sale-price">${salla.money(product.sale_price)}</span>
                   <span class="product-regular-price">${salla.money(product.regular_price)}</span>` : `<span class="product-price">${salla.money(product.price)}</span>`;
        infoCard.innerHTML = `
