    ...
    async fetchCategoryPage(catObj, offset, limit) {
      if (catObj.slug === "trending-now") {
        try {
          ...
          const hasMore = typeof result?.hasMore === "boolean" ? result.hasMore : objectIDs.length === limit;
      ...
      if (catObj.ids && catObj.ids.length > 0) {
        const categoryIdFetches = catObj.ids.map(
          (numericID) => redisService.getCategoryPageById(numericID, offset, limit).catch(() => ({ objectIDs: [], hasMore: false }))
        );
        try {
...
`;
  ...
  var GomlaModalRerank = class {
    ...
    buildInlineTrackSignature(track, currentProductId, orderedIds) {
      const itemIds = Array.from(track.querySelectorAll(".gomla__carousel-item .gomla__product-card[data-product-id]")).map((card) => this.normalizeProductId(card.dataset.productId)).filter(Boolean);
    ...
    reorderInlineItemsMatchedThenUnmatched(track, orderedIds) {
      ...
      const itemsByProductId = /* @__PURE__ */ new Map();
      items.forEach((item) => {
    ...
    getCurrentProductId() {
      ...
      const fromProductElement = this.normalizeProductId(
        document.querySelector('.product-entry[id^="product-"], #product-entry[id^="product-"], [id^="product-"]')?.id?.replace("product-", "")
      );
