(()=>{var E=class{constructor(){this.baseUrl="https://me-central2-gtm-5v2mhn4-mwvlm.cloudfunctions.net/function-2",this.maxRetries=2,this.headers={Accept:"application/json","Cache-Control":"public, max-age=3600"},this.cache=new Map,this.fallbackEnabled=!0}async getProducts(e,t,r=0,s=12){if(!t)return null;let i=`${e}:${t}:${r}:${s}`;if(this.cache.has(i))return this.cache.get(i);let o=e==="category"?"catID":"tagID",a=e==="category"?"categoryById":"tagById",n=`${this.baseUrl}/?type=${a}&${o}=${encodeURIComponent(t)}&offset=${r}&limit=${s}`,c=null,l=!1;try{let u=new AbortController,h=setTimeout(()=>u.abort(),2e3),g=await fetch(n,{method:"GET",headers:this.headers,signal:u.signal});if(clearTimeout(h),!g.ok)throw new Error(`HTTP error: ${g.status}`);if(c=await g.json(),c?.objectIDs?.length)return this.cache.set(i,c),c;let f=Number(c?.totalObjectIDs??c?.count??0);r===0&&Array.isArray(c?.objectIDs)&&c.objectIDs.length===0&&f>0&&(l=!0)}catch{}if(!c?.objectIDs?.length&&this.fallbackEnabled)try{l&&await new Promise(h=>setTimeout(h,750));let u=await fetch(n,{method:"GET",headers:this.headers});if(u.ok&&(c=await u.json(),c?.objectIDs?.length))return this.cache.set(i,c),c}catch{}return c||{objectIDs:[],hasMore:!1}}async getRecommendations(e){if(!e)return[];let t=`recommendations:${e}`;if(this.cache.has(t))return this.cache.get(t);let r=`${this.baseUrl}/?type=recommendations&objectID=${encodeURIComponent(e)}`;try{let s=new AbortController,i=setTimeout(()=>s.abort(),3e3),o=await fetch(r,{method:"GET",headers:this.headers,signal:s.signal});if(clearTimeout(i),!o.ok)return[];let a=await o.json(),n=Array.isArray(a?.relatedProductIDs)?a.relatedProductIDs:[];return this.cache.set(t,n),n}catch{return[]}}async getFrequentlyBought(e){if(!e)return[];let t=`frequently-bought:${e}`;if(this.cache.has(t))return this.cache.get(t);let r=`${this.baseUrl}/?type=frequentlyBought&objectID=${encodeURIComponent(e)}`;try{let s=new AbortController,i=setTimeout(()=>s.abort(),3e3),o=await fetch(r,{method:"GET",headers:this.headers,signal:s.signal});if(clearTimeout(i),!o.ok)return[];let a=await o.json(),n=Array.isArray(a?.frequentlyBoughtIDs)?a.frequentlyBoughtIDs:[];return this.cache.set(t,n),n}catch{return[]}}async getCategoriesFromRedis(){let e="all-categories";if(this.cache.has(e))return this.cache.get(e);try{let t=new AbortController,r=setTimeout(()=>t.abort(),5e3),s=await fetch(`${this.baseUrl}/?type=categories`,{method:"GET",headers:this.headers,signal:t.signal});if(clearTimeout(r),!s.ok)return[];let o=(await s.json()).categories||[];return Array.isArray(o)&&o.length>0&&this.cache.set(e,o),o}catch{return[]}}async getGlobalProducts(e=0,t=12){let r=`global-products:${e}:${t}`;if(this.cache.has(r))return this.cache.get(r);try{let s=new AbortController,i=setTimeout(()=>s.abort(),3e3),o=`${this.baseUrl}/?type=categoryById&catID=trending-now&offset=${e}&limit=${t}`,a=await fetch(o,{method:"GET",headers:this.headers,signal:s.signal});if(clearTimeout(i),!a.ok)return{objectIDs:[],hasMore:!1};let n=await a.json(),c={objectIDs:n.objectIDs||[],hasMore:n.hasMore||!1};return this.cache.set(r,c),c}catch{return{objectIDs:[],hasMore:!1}}}async getCategoryPageById(e,t=0,r=12){return this.getProducts("category",e,t,r)}async getCategoryProducts(e,t,r){return this.getProducts("category",e,t,r)}async getTagProducts(e,t,r){return this.getProducts("tag",e,t,r)}},m=new E;var x=class extends HTMLElement{constructor(){super(),this.initialized=!1,this.productIds=[],this.structureReady=!1,this.cartListenerBound=!1,this.reloadScheduled=!1,this.reloadTimer=null}isCartPage(){return window.location.pathname.includes("/cart")&&document.querySelector('form[id^="item-"]')!==null}scheduleReload(){this.isCartPage()&&(this.reloadScheduled||(this.reloadScheduled=!0,this.reloadTimer&&clearTimeout(this.reloadTimer),this.reloadTimer=setTimeout(()=>{console.log("[CartAddonsSlider] Reloading cart page to show added item"),window.location.reload()},1e3)))}setupCartRefreshListener(){if(this.cartListenerBound||!this.isCartPage())return;this.cartListenerBound=!0;let e=()=>this.scheduleReload();window.salla?.cart?.event?.onItemAdded&&(window.salla.cart.event.onItemAdded(()=>this.scheduleReload()),console.log("[CartAddonsSlider] Listening to salla.cart.event.onItemAdded()")),document.addEventListener("salla::cart::item.added",e),console.log("[CartAddonsSlider] Listening to salla::cart::item.added")}connectedCallback(){this.ensureStructure(),this.setupCartRefreshListener(),!this.initialized&&this.getHighestValueItemFromDOM().then(e=>e?this.fetchFrequentlyBoughtProducts(e):[]).then(()=>{this.renderProductList(),setTimeout(()=>{this.initializeSlider()},1e3)}).catch(e=>{console.error("[CartAddonsSlider] Error loading products:",e)})}ensureStructure(){if(this.structureReady)return;let e=this.getTitleText(),t=this.querySelector(".cart-addons-title");if(t)t.textContent=e;else{let r=document.createElement("h3");r.className="cart-addons-title",r.textContent=e,this.appendChild(r)}if(!this.querySelector(".frequently-bought-container")){let r=document.createElement("div");r.className="frequently-bought-container",this.appendChild(r)}this.structureReady=!0}getTitleText(){let e="pages.cart.frequently_bought_together",t=window.salla?.lang?.get?.(e),r=typeof t=="string"&&t!==e?t.trim():"";if(r)return r;let s=(document.documentElement.getAttribute("lang")||"").toLowerCase().slice(0,2),i={ar:"\u0623\u0636\u064A\u0641\u064A \u0645\u0646\u062A\u062C \u0648\u0627\u062D\u0635\u0644\u064A \u0639\u0644\u0649 \u062E\u0635\u0645 15%",en:"Add a product & get 15% off"};return i[s]||i.en}async getHighestValueItemFromDOM(){try{let e=document.querySelectorAll('form[id^="item-"]');if(!e||e.length===0)return console.log("[CartAddonsSlider] No cart items found in DOM"),null;let t=null,r=0;return e.forEach(s=>{try{let i=s.getAttribute("id")||"",o=i.replace("item-",""),a=s.querySelector('a[href*="/p"]');if(!a){console.log("[CartAddonsSlider] No product link found in form",i);return}let n=a.getAttribute("href"),c=n.match(/\/p(\d+)(?:$|\/|\?)/);if(!c||!c[1]){console.log("[CartAddonsSlider] Could not extract product ID from URL",n);return}let l=c[1],u=s.querySelector(".item-total");if(u){let g=(u.textContent||u.innerText||"0").replace(/[^\d.,٠١٢٣٤٥٦٧٨٩]/g,"").replace(/٠/g,"0").replace(/١/g,"1").replace(/٢/g,"2").replace(/٣/g,"3").replace(/٤/g,"4").replace(/٥/g,"5").replace(/٦/g,"6").replace(/٧/g,"7").replace(/٨/g,"8").replace(/٩/g,"9"),f=parseFloat(g.replace(",","."))||0;console.log(`[CartAddonsSlider] Item ${o}: Product ID ${l}, Total: ${f}`),!isNaN(f)&&f>r&&(r=f,t={itemId:o,productId:l,total:f})}}catch(i){console.error("[CartAddonsSlider] Error processing item form:",i)}}),t?(console.log("[CartAddonsSlider] Highest value item:",t),t.productId):null}catch(e){return console.error("[CartAddonsSlider] Error extracting cart data from DOM:",e),null}}async fetchFrequentlyBoughtProducts(e){try{console.log("[CartAddonsSlider] Fetching frequently bought products for:",e);let t=await m.getFrequentlyBought(e);return t&&t.length>0?(console.log("[CartAddonsSlider] Found frequently bought products:",t),this.productIds=t.map(r=>String(r)).slice(0,8),this.productIds):(console.log("[CartAddonsSlider] No frequently bought products found"),this.productIds=[],[])}catch(t){return console.error("[CartAddonsSlider] Error fetching frequently bought products:",t),this.productIds=[],[]}}renderProductList(){if(!this.productIds||this.productIds.length===0){console.log("[CartAddonsSlider] No products to render, hiding component"),this.style.display="none";return}console.log("[CartAddonsSlider] Rendering product list with IDs:",this.productIds);let e=this.querySelector(".frequently-bought-container");if(!e){console.error("[CartAddonsSlider] Container not found");return}let t=document.createElement("salla-products-list");if(t.setAttribute("source","selected"),t.setAttribute("loading","lazy"),t.setAttribute("source-value",JSON.stringify(this.productIds)),t.setAttribute("class","s-products-list-vertical-cards"),e.innerHTML="",e.appendChild(t),!this.querySelector(".touch-indicator")){let r=document.createElement("div");r.classList.add("touch-indicator"),this.appendChild(r)}console.log("[CartAddonsSlider] Product list rendered")}applyDiscount(){let t=()=>{let s=this.querySelectorAll(".s-product-card-sale-price h4");return s.length===0?!1:(s.forEach(i=>{if(i.dataset.discounted)return;let o=i.textContent||"",a=o.replace(/[^\d.,٠١٢٣٤٥٦٧٨٩]/g,"").replace(/٠/g,"0").replace(/١/g,"1").replace(/٢/g,"2").replace(/٣/g,"3").replace(/٤/g,"4").replace(/٥/g,"5").replace(/٦/g,"6").replace(/٧/g,"7").replace(/٨/g,"8").replace(/٩/g,"9"),n=parseFloat(a.replace(",","."));if(!isNaN(n)){let c=(n*.85).toFixed(2);i.textContent=o.replace(/[\d.,٠١٢٣٤٥٦٧٨٩]+/,c),i.dataset.discounted="true"}}),!0)};if(t())return;let r=new MutationObserver(()=>{t()&&r.disconnect()});r.observe(this,{childList:!0,subtree:!0}),setTimeout(()=>r.disconnect(),1e4)}initializeSlider(){try{let e=this.querySelector("salla-products-list");if(!e){console.log("[CartAddonsSlider] Products list not found");return}e.style.opacity="1",window.salla?.event?.dispatch&&window.salla.event.dispatch("twilight::mutation"),this.applyDiscount(),this.initialized=!0,console.log("[CartAddonsSlider] Slider initialized")}catch(e){console.error("[CartAddonsSlider] Failed to initialize cart addons slider:",e)}}};customElements.get("cart-addons-slider")||customElements.define("cart-addons-slider",x);var C=class extends HTMLElement{constructor(){super(),this.swiper=null,this.productIds=[],this.initialized=!1}async initialize(e,t={}){if(this.initialized){console.warn("[AlgoliaSlider] Already initialized");return}this.productIds=e.slice(0,21);let{title:r="\u0645\u0646\u062A\u062C\u0627\u062A \u0642\u062F \u062A\u0639\u062C\u0628\u0643",displayAllUrl:s="",filterStock:i=!0,maxInStock:o=15}=t;this.innerHTML=`
        ...
        `}async initializeCategorySections(){try{this.seenProductIds.clear();let e=this.state.categories,t=4,r="2000px 0px",s=8e3,i=document.body.classList.contains("index"),o=typeof IntersectionObserver<"u",a=async p=>{let w=e[p],A=await this.collectUniqueIdsForCategory(w,6);this.renderCategorySlider(w,A)};if(!i||!o){for(let p=0;p<e.length;p++)await a(p);return}let n=new Array(e.length).fill(!1),c=0,l=!1,u=null,h=Promise.resolve(),g=()=>{h=h.then(async()=>{for(;c<e.length&&!(!(c<t)&&!n[c]&&!l);)await a(c),c++;c>=e.length&&u&&(u.disconnect(),u=null)}).catch(p=>{this.handleInitError(p)})};for(let p=0;p<Math.min(t,e.length);p++)n[p]=!0;u=new IntersectionObserver(p=>{for(let w of p){if(!w.isIntersecting)continue;let A=w.target?.getAttribute("data-category-index");if(A==null)continue;let I=Number(A);Number.isNaN(I)||n[I]||(n[I]=!0,g())}},{rootMargin:r});for(let p=t;p<e.length;p++){let w=this.querySelector(`[data-category="${e[p].slug}"]`);if(!w){n[p]=!0;continue}w.setAttribute("data-category-index",String(p)),u.observe(w)}let f=()=>{setTimeout(()=>{l=!0,u&&(u.disconnect(),u=null),g()},s)};document.readyState==="complete"?f():window.addEventListener("load",f,{once:!0}),g()}catch(e){this.handleInitError(e)}}async collectUniqueIdsForCategory(e,t){let r=[],s=0,i=!0;for(;r.length<t&&i;){let o=await this.fetchCategoryPage(e,s,this.state.productsPerPage),a=o.objectIDs||[];if(!a.length)break;for(let n of a)if(!this.seenProductIds.has(n)&&(this.seenProductIds.add(n),r.push(n),r.length>=t))break;if(r.length>=t||(typeof o.hasMore=="boolean"?i=o.hasMore:i=a.length===this.state.productsPerPage,!i))break;s+=this.state.productsPerPage}return r}async fetchCategoryPage(e,t,r){if(e.slug==="trending-now")try{let s=await m.getGlobalProducts(t,r),i=s?.objectIDs||[],o=typeof s?.hasMore=="boolean"?s.hasMore:i.length===r;return{objectIDs:i,hasMore:o}}catch{return{objectIDs:[],hasMore:!1}}if(e.ids&&e.ids.length>0){let s=e.ids.map(i=>m.getCategoryPageById(i,t,r).catch(()=>({objectIDs:[],hasMore:!1})));try{let i=await Promise.all(s),o=i.flatMap(n=>n&&n.objectIDs?n.objectIDs:[]),a=i.some(n=>typeof n?.hasMore=="boolean"?n.hasMore:Array.isArray(n?.objectIDs)&&n.objectIDs.length===r);return{objectIDs:o,hasMore:a}}catch{return{objectIDs:[],hasMore:!1}}}return{objectIDs:[],hasMore:!1}}renderProductSliders(e){this.state.categories.forEach(t=>{let r=t.slug;if(!Object.prototype.hasOwnProperty.call(e,r))return;let s=e[r]||[];this.renderCategorySlider(t,s)})}renderCategorySlider(e,t){let r=e.slug,s=this.querySelector(`#products-${r}`);if(s)if(s.innerHTML="",t.length>0){let i=document.createElement("salla-products-slider");i.setAttribute("source","selected"),i.setAttribute("source-value",JSON.stringify(t)),i.setAttribute("limit",String(t.length)),i.setAttribute("slider-id",`slider-${r}`),i.setAttribute("block-title"," "),i.setAttribute("arrows","true"),i.setAttribute("rtl","true"),s.appendChild(i),setTimeout(()=>{i.querySelectorAll(".s-product-card-content-sub").forEach(a=>{a.children.length>1&&(a.style.display="flex",a.style.alignItems="center",a.style.justifyContent="space-between",a.style.flexWrap="nowrap",a.style.width="100%",a.style.overflow="visible")})},500)}else s.innerHTML='<div style="text-align: center; padding: 1rem;">\u0644\u0627 \u062A\u0648\u062C\u062F \u0645\u0646\u062A\u062C\u0627\u062A \u0644\u0639\u0631\u0636\u0647\u0627 \u0641\u064A \u0647\u0630\u0647 \u0627\u0644\u0641\u0626\u0629.</div>'}handleInitError(e){this.innerHTML=`
            <div class="category-filter">
